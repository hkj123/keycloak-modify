...
//=================改装选择URI====START=====================
module.factory('ResourceServerUri', function($resource) {
    const packageUrl = `http://qloudmnt.pditdap.service.sd`;
    return $resource(packageUrl+"/endpoints/query/baseurls", {
        realm : '@realm',
    }, {
        'search' : {url: packageUrl+"/endpoints/query/baseurls", method : 'GET'},
    });
});
//=================改装选择URI====END=====================

module.controller('ResourceServerResourceDetailCtrl', function($scope,$cookies,$cookieStore, $http, $route, $location, realm, ResourceServer, ResourceServerUri,client, ResourceServerResource, ResourceServerScope, AuthzDialog, Notifications) {
    $scope.realm = realm;
    $scope.client = client;

    //=================改装选择URI====START=====================
    $scope.isHand= true; // true:手写，false：搜索并选择
    // 测试是否可以调通接口
    ResourceServerUri.search({pattern: "", count: 20}, function(response) {
        if(response['data'] instanceof Array && response['data'].length){
            $scope.isHand = false; // 改为搜索并选择
        }
    });
    ...
    $scope.urisUiSelect = {
        ...
        query: function (query) {
            ...
            ResourceServerUri.search({pattern: query.term.trim(), count: 20}, function(response) {
                console.log('===response===')
                console.log(response)
                let results = [];
                if(response['data'].length){
                    $scope.isHand = false; // 改为搜索选择
    ...
    // 选择URI事件
    $scope.selectUrl = function(user) {
        if (!user || !user.url) {
            return;
        }
        $scope.selectedUrl = null;
        // 判断是否存在
        const existed = $scope.resource.uris.includes(user['url']);
        if (existed == false) {
            $scope.resource.uris.push(user['url']);
        }
    }
    //=================改装选择URI====END=====================
